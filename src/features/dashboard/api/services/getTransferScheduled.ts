...
export const getTransfersScheduled = async (): Promise<BaseResponse<ScheduledTransfersResponse>> => {
  const api = await createServerApi()
  const { data } = await api.get<BaseResponse<ScheduledTransfersResponse>>('/financial/transfers/scheduled')
  return data
}
