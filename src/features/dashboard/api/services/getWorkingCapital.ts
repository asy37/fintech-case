...
export const getWorkingCapital = async (): Promise<BaseResponse<WorkingCapitalResponse>> => {
  const api = await createServerApi()
  const { data } = await api.get<BaseResponse<WorkingCapitalResponse>>('/financial/working-capital')
  return data
}